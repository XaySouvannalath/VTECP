...
$sql3 = "select DISTINCT ct.CourseTypeID,ct.CourseTypeName  from tb_coursetype ct, tb_course c where ct.CourseTypeID = c.CourseTypeID and c.courseID  in (select courseid  from tb_training where positionid = '" . $_GET["PositionID"] . "')";
$sql4 = "select T.CourseID, C.CourseName, CT.CourseTypeID from tb_course C, tb_training T, tb_coursetype CT where C.CourseID = T.CourseID and C.CourseTypeID = CT.CourseTypeID and CT.CourseTypeID = 'CT001' and T.PositionID = '" . $_GET["PositionID"] . "'";
...
$resultCT = mysqli_query($connect, $sql3);
//echo $sql;        
  ...
  font-size: 90%;   
    ...
    <body>
        ...
        <div class="container" id="content">
            ...
            <div class="row">
                <div class="">
                    <?php while($rowCT = mysqli_fetch_array($resultCT))
                    {
                   ?>     
                   
                    <div class="panel modernbody">
                        <center><div class="panel panel-heading modernfont" id="navcolor"> <?php echo $rowCT['CourseTypeName']; ?></div></center>
                        
                    <div class="panel-body">
                        <table class="table table-bordered">
                            <tr>
                            <th align="middle" class="tablehead text-center"> No. </th> 
                             <th class="tablehead text-left">Course Name</th> 
                            </tr>
                            <?php 
 $sql4 = "select T.CourseID, C.CourseName, CT.CourseTypeID from tb_course C, tb_training T, tb_coursetype CT where C.CourseID = T.CourseID and C.CourseTypeID = CT.CourseTypeID and CT.CourseTypeID = '" .$rowCT['CourseTypeID']."' and T.PositionID = '" . $_GET["PositionID"] . "'";
                        $resultCD = mysqli_query($connect, $sql4);
                        
                        $i = 1;
                            while($rowCD = mysqli_fetch_array($resultCD)){ ?>
                            <tr>
                                <td class="text-center" id="whitefont"><?php echo $i; ?></td>
                                <td class="text-left" id="whitefont" > <?php echo $rowCD['CourseName']; ?> </td>
                            </tr>
                            <?php $i = $i+1; } ?>
                        </table>
                    </div>
